...
/**
 *  This macro displays the PID controller info: Target, Error, Input and
 *  Output on the LCD display at the given line.
 *
 *  @param p Points to the PID Control structure.
 *  @param n Specifies the LCD line number.
 */
#define PIDCtrlDisplayInfo(n,p) nxtDisplayTextLine(n, "T=%5.1f,E=%5.1f",    \
                                                   (p).setPoint,            \
                                                   (p).prevError);          \
                                nxtDisplayTextLine((n)+1, "I=%5.1f,O=%5.1f",\
                                                   PIDCtrlGetInput(p),      \
                                                   (p).prevOutput)
